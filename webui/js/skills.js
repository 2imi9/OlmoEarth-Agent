...
const SKILLS = [
  ...
  { n: 6,  slug: 'baseline-compare',  cat: 'Run',       icon: 'compare',     desc: 'OlmoEarth vs a baseline foundation model (e.g. AlphaEarth), head-to-head on transfer regions.', pink: true, ex: 'Compare OlmoEarth vs AlphaEarth for land cover in a region where AlphaEarth struggles.' },
...
const SKILL_TOOLS = {
  ...
  5: `olmoearth_change_detect (Engine A, composes #4; enforce_min_3_dates · diff_layers) · olmoearth_trace_shifts (Engine A+: date-ordered, legend-calibrated shift trace) · Engine B: out-of-process python -m oejc.skill (JEPA latent residual)`,
...
const SKILL_IO = {
  ...
  5:  { in: 'Engine A: 3+ dated per-layer summaries. A+: 3-8 result ids of one model over one area. Engine B: two co-registered S2 GeoTIFFs (or AOI + 2 dates).', out: 'A: trajectory metrics (refuses <3 dates). A+: per-step correlation + per-point trajectories + legend-calibrated shifts. B: a georeferenced change heatmap + %-area-changed + top-k GeoJSON.' },
