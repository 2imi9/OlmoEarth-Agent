...
const SKILLS = [
  ...
  { n: 17, slug: 'rslearn',           cat: 'Configure', icon: 'sliders',     desc: 'rslearn made approachable: recommend a full training setup from a plain-language goal, and validate a config (encoder/decoder/head shapes, task↔label-type, bands) before a multi-hour run - plus run-the-pipeline guidance.', ex: 'I want to map flooded buildings after a storm from Sentinel-2 - set up the rslearn task + model for me, then check the config before I train.' },
];
...
const SKILL_TOOLS = {
  ...
  17: `olmoearth_load_skill (run the 4-stage pipeline + fit/predict) · olmoearth_rslearn_recommend (goal → explained setup) · olmoearth_rslearn_validate (shape / label-type / band checks before training)`,
};
