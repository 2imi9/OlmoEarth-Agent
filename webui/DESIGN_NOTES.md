...
- **Comparisons panel** (`js/compares.js`), **done**: a sidebar section
  (parallel to Chats/Projects) that stores difference-scan results (real stats
  + diff grid) in `localStorage` and re-opens them in a modal; saved via an
  `oe:save-comparison` event so `viz.js` need not import the panel.
