...
| C9 | Skill catalog | 16 skills in `SKILLS.md` (Prep / Configure / Run / Analyze / Integrate / Report). 15 implemented in-repo; skill #5 `olmoearth-change-detection` also has an out-of-process JEPA engine backed by `2imi9/olmoearth-jepa-change`. Vendored #1-#3 (the three `vendor/olmoearth-skills` packages: data-prep, studio-job-config, embeddings) via submodule. Earlier catalog versions split data-prep into #1/#2, embeddings into #4/#17, and change-detection into #6/#19; those were merged. |
