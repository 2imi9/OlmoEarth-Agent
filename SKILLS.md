...
- `olmoearth_compare_results`: a **quantitative** two-result comparison with no ground truth. Samples both rasters on a grid (pointwise `pixel-value`) over their shared extent and returns model-vs-model agreement -- mean / mean-absolute difference, RMSE-between-models, correlation, and an agreement fraction (regression) or class agreement (classification). This is the numeric counterpart to the side-by-side raster preview; it reports *divergence between two outputs*, not accuracy (which needs labels -> skill #7 `olmoearth-evaluate`).
