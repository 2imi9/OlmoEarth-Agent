...
| Harness | In-repo typed single-agent harness, modeled after [ByteDance DeerFlow v2](https://github.com/bytedance/deer-flow): lead agent + ordered middleware seams, with subagents parked until measured isolation/parallelism value exists. OpenClaw contributes soul/skills/config and eligibility patterns, **not** the runtime. Decision: [`docs/agent-runtime-baseline.md`](docs/agent-runtime-baseline.md). |
