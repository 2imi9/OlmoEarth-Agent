...
**Status:** v1.1.0, 2026-05-31. All 18 in-repo skills implemented (v1.0 shipped #1-#15; #16 `olmoearth-litsearch` + #17 `olmoearth-automate` in v1.1.0; #18 `olmoearth-negative-sampler` added post-1.1), plus #19 `olmoearth-latent-change` as an **out-of-process** companion in the separate [`olmoearth-jepa-change`](https://github.com/2imi9/olmoearth-jepa-change) repo; runs live against the Studio API and a local or hosted (Claude/ChatGPT/Gemini) LLM. See §6 and [`CHANGELOG.md`](CHANGELOG.md).
...
**Skill catalog:** [`SKILLS.md`](SKILLS.md): 19 skills (Prep / Configure / Run / Analyze / Integrate / Report); #19 `olmoearth-latent-change` is out-of-process (separate repo). The agent ships skills one PR at a time.
