        ...
        RegisteredTool(
            spec=ToolSpec(
                ...
                description=(
                    ...
                    "for a user who will RUN THE TRAINING THEMSELVES with the rslearn "
                    "pipeline (local or their own compute). Use ONLY when the user is "
                    "clearly operating rslearn directly — e.g. 'set up rslearn', "
                    "'configure the fine-tune pipeline / model.yaml', 'I'm training "
                    "this myself on my GPU'. DO NOT use this to create or configure a "
                    "model in OlmoEarth Studio (the hosted platform, where Ai2 runs "
                    "the training): for any plain 'set up / configure / build a model "
                    "to map/detect/measure X' request, load the "
                    "`olmoearth-studio-job-config` skill and walk the Studio wizard "
                    "instead — never ask about local compute. Returns the rslearn task "
        ...
        RegisteredTool(
            spec=ToolSpec(
                ...
                description=(
                    ...
                    "for a user running rslearn training THEMSELVES. Use after "
                    "`olmoearth_rslearn_recommend`, or when the user explicitly wants "
                    "the rslearn config ('write the model.yaml', 'give me the rslearn "
                    "YAML'). NOT for configuring a model in OlmoEarth Studio (Ai2 "
                    "trains it) — load `olmoearth-studio-job-config` for that. Mirrors "
                    "the proven OlmoEarth template (MultiTaskModel "
